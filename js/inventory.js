...
function renderInventory(page, pageSize) {
  if (page) invPage = page;
  if (pageSize) invPageSize = pageSize;
  ...
  if (!filtered.length) {
    ...
    renderPagination('invPagination', 0, 1, invPageSize, renderInventory);
    return;
  }

  const paged = paginateArray(filtered, invPage, invPageSize);
  tbl.innerHTML = paged.map(i => {
  ...
  renderPagination('invPagination', filtered.length, invPage, invPageSize, renderInventory);
}
